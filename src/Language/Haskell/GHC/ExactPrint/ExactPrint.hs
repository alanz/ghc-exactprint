...
enterAnn !(Entry anchor' trailing_anns cs flush canUpdateAnchor) a = do
  ...
  let (edp, medr) = case med of
        ...
        Just (EpaSpan (GeneratedSrcSpan r)) -> panic $ "enterAnn: GeneratedSrcSpan:" ++ show r
...
instance ExactPrint (IE GhcPs) where
  ...
  exact (IEThingAll (IEThingAllExt depr ns_spec op dd cp) thing doc) = do
    depr' <- markAnnotated depr
    thing' <- markAnnotated thing
    op' <- markEpToken op
    ...
    cp' <- markEpToken cp
    doc' <- markAnnotated doc
    return (IEThingAll (IEThingAllExt depr' ns_spec' op' dd' cp') thing' doc')
...
getExtraDP :: (Monad m) => EP w m (Maybe EpaLocation)
getExtraDP = gets uExtraDP

setExtraDP :: (Monad m) => Maybe EpaLocation -> EP w m ()
setExtraDP md = do
  debugM $ "setExtraDP:" ++ show md
  modify' (\s -> s {uExtraDP = md})

getExtraDPReturn :: (Monad m) => EP w m (Maybe (SrcSpan, DeltaPos))
getExtraDPReturn = gets uExtraDPReturn

setExtraDPReturn :: (Monad m) => Maybe (SrcSpan, DeltaPos) -> EP w m ()
